...
public class GradeList {
      ...
      // -수정할 과목을 찾아서 점수를 수정. -> 찾을 과목명, 수정할 점수
      // -수정할 과목을 찾아서 과목과 점수를 수정.  -> 변경전 과목명,변경 후 과목명,수정할 점수
      // -수정할 데이터가 저장되어있는 인덱스를 지정하여 과목명을 수정 
      // -수정할 데이터가 저장되어있는 인덱스를 지정하여 점수를 수정 
      // -수정할 데이터가 저장되어있는 인덱스를 지정하여 과목명과 점수를 수정 
      ...
      public double getTotal() {// 과목 배열에 있는 모든 점수들의 합
    	  double tot =0;
    	  for(int i =0; i<length(); i++) {
         ...
         // 제시한 score 미만에 해당하는 과목 정보를 구함
    	  ...
    	  for(int i =0; i<length(); i++) {
    		  if(this.gList[i].getScore()<score) {
    			  result = Arrays.copyOf(result,result.length+1);//동적배열
      ...
      public String getTop() {//최고 득점 과목을 구함
      ...
      public String[] getTop(int count) {
    	  ...
    	  for(int i =0; i <tArr.length-1; i++) {
    		  for(int j=i+1; j<tArr.length; j++) {  //탐색해야하는 배열크기만큼...
	...
	//기존에 정렬과 관련된 기능은 다음의 sort를 사용하는 것으로 변경한다.
	public void sort() {
		//오름차순으로 정렬
